...
def extrair_paginas_filtradas(caminho_pdf: Path, caminho_saida: Path, logger: logging.Logger) -> bool:
    ...
    try:
        ...
        if paginas_encontradas == 0:
            logger.warning(f"No valid pages: {caminho_pdf.name}")
            return False
        ...
        logger.error(f"❌ Error: {caminho_pdf.name}: {e}")
        return False
...
def processar_limpeza(origem: Path, limpos: Path, logger: logging.Logger, 
    ...
    logger.info(f"Total files: {len(arquivos)}")
    
    for i, (file, caminho_origem) in enumerate(arquivos, 1):
        logger.info(f"[{i}/{len(arquivos)}] {file}")
...
def consolidar_dados(limpos: Path, logger: logging.Logger,
    ...
    for caminho in arquivos:
        try:
            ...
            logger.info(f"  ✅ {caminho.name}: {n_bene} beneficiaries | gen: {'✅' if tem_ger else '—'}")
        
        except Exception as e:
            logger.error(f"  ❌ Error {caminho.name}: {e}")
...
def imprimir_resumo(df: pd.DataFrame, logger: logging.Logger):
    ...
    if not df_comp.empty:
        ...
        logger.info(f"Value: R$ {df_comp['Valor Compensado (R$)'].sum():,.2f}")
...
def executar_pipeline(debug: bool = True, salvar: bool = True, 
    ...
    logger.info("="*60)
    logger.info("🚀 COPEL PIPELINE v5.1 - MÊS CORRIGIDO")
    logger.info("="*60)
